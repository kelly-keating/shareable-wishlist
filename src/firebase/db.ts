...
export function watchYourList(
  cb: (list: Wishlist) => void,
  ohNoFn: () => void
): void {
  const { uid } = getUser()
  if (!uid) throw new Error ('how did you get here')

  const theRef = ref(db, 'lists/' + uid)
...
export function createYourList() {
  const { uid } = getUser()
  if (!uid) throw new Error ('how did you get here')
  const theRef = ref(db, 'lists/' + uid)
